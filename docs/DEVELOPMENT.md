...
Release signing requires repository secrets `KEYSTORE_BASE64`, `KEYSTORE_PASSWORD`, `KEY_ALIAS`, and `KEY_PASSWORD`. The workflow is migrated, but those secret values must be configured in `enclaro/kanarek` before the first standalone release; GitHub does not expose existing secret values for copying from another repository.
